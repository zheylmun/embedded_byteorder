...
/// Extends [`Read`] with methods for reading numbers. (For `embedded-io`.)
///
...
/// [`Read`]: https://docs.rs/embedded-io/0.7/embedded_io/trait.Read.html
pub trait ReadBytesExt: Read {
    ...
    ///
    /// ```rust
    /// use byteorder_embedded_io::ReadBytesExt;
    ///
    /// let mut rdr = &[2u8, 5][..];
    ...
    /// # Examples
    ///
    /// Read a sequence of big-endian single-precision floating point numbers
    /// from a `Read`:
    ///
    /// ```rust
    /// use core::f32;
    ...
    /// # Examples
    ///
    /// Read a sequence of big-endian double-precision floating point numbers
    /// from a `Read`:
    ///
    /// ```rust
    /// use core::f64;
...
/// Extends [`Write`] with methods for writing numbers. (For `embedded-io`.)
///
/// Most of the methods defined here have an unconstrained type parameter that
/// must be explicitly instantiated. Typically, it is instantiated with either
/// the [`BigEndian`] or [`LittleEndian`] types defined in this crate.
...
///
/// ```rust
/// use byteorder_embedded_io::{BigEndian, WriteBytesExt};
///
/// let mut buf = [0u8; 4];
/// {
///     let mut wtr = &mut buf[..];
...
/// [`BigEndian`]: enum.BigEndian.html
/// [`LittleEndian`]: enum.LittleEndian.html
/// [`Write`]: https://docs.rs/embedded-io/0.7/embedded_io/trait.Write.html
pub trait WriteBytesExt: Write {
    ...
    ///
    /// ```rust
    /// use byteorder_embedded_io::WriteBytesExt;
    ///
    /// let mut buf = [0u8; 2];
    /// {
    ///     let mut wtr = &mut buf[..];
    ...
    /// }
    /// assert_eq!(buf, [0x48, 0xc5, 0x74, 0x62, 0xe9]);
    /// ```
    #[inline]
    ...
    /// }
    /// assert_eq!(buf, [0xf3, 0x64, 0xf4, 0xd1, 0xfd, 0xb0, 0x81]);
    /// ```
    #[inline]
    ...
    /// # Examples
    ///
    /// Write a big-endian single-precision floating point number to a
    /// `Write`:
    ///
    /// ```rust
    /// use core::f32;
    ...
    /// # Examples
    ///
    /// Write a big-endian double-precision floating point number to a
    /// `Write`:
    ///
    /// ```rust
    /// use core::f64;
